...
class SpatialEncoder(nn.Module):
    ...
    def __init__(self, in_channels, embed_dim):
        ...
        self.down1 = nn.Sequential(
            ConvResidualBlock(self.base_ch, stride=1, dropout=self.down1_dropout),
            ConvResidualBlock(self.base_ch, stride=1, dropout=self.down1_dropout)
        )
        ...
        self.down2 = nn.Sequential(
            ...
            ConvResidualBlock(embed_dim, stride=1, dropout=self.down2_dropout)
        )
...
class SpatialTemporalDecoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # f: (B, T, E, H’, W’)
        # Fuse over time with a simple reduction:
        f = f.mean(dim=1)       # (B, E, H’, W’)
